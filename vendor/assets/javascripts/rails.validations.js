    ...
    'Formtastic::FormBuilder': {
      add: function(element, settings, message) {
        if (element.data('valid') !== false) {
          ...
          var errorElement = $('<span class="' + settings.inline_error_class + '">' + message + '</span>');
          wrapper.find("label").append(errorElement);
        } else {
          element.parent().find('span.' + settings.inline_error_class).text(message);
        }
      },
      remove: function(element, settings) {
        var wrapper = element.closest('li.error');
        wrapper.removeClass('error');
        var errorElement = wrapper.find('span.' + settings.inline_error_class);
        errorElement.remove();
      }
    },
    'NestedForm::Builder': {
